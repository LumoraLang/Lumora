...
void Pipeline::log(std::string_view msg) const {
    if (m_opts.verbose) std::cerr << "[lumora] " << msg << "\n";
}
...
std::filesystem::path Pipeline::outputPath(const std::filesystem::path& src) const {
...
bool Pipeline::run() {
    ...
    for (auto& grp : m_cfg.groups) {
        for (auto& file : grp.files) {
            ...
            if (!res.success) {
                std::cerr << "error: compilation failed for " << file << "\n";
                allOk = false;
            } else {
    ...
    for (auto& optStep : m_cfg.optSteps) {
        if (!runOpt(optStep)) {
            std::cerr << "error: opt step failed for " << optStep.input << "\n";
            return false;
        }
    }
    ...
    for (auto& lnkStep : m_cfg.linkSteps) {
        if (!runLink(lnkStep)) {
            std::cerr << "error: link step failed\n";
            return false;
        }
    }
...
CompileResult Pipeline::compileFile(const std::filesystem::path& srcPath) {
    ...
    if (m_opts.dumpTokens) {
        ...
        for (auto& t : tokens)
            std::cout << t.loc.line << ":" << t.loc.col << " " << t.kindName() << " '" << t.raw << "'\n";
        result.success = true;
        return result;
    }
    ...
    if (!out) {
        std::cerr << "error: cannot write " << result.irPath << "\n";
        return result;
    }
    out << result.irOutput;
...
bool Pipeline::runLink(const LinkStep& step) {
    ...
    for (auto& f : step.flags)  cmd += " " + f;
    for (auto& i : step.inputs) cmd += " " + i;
    cmd += " -o " + step.output;
    for (auto& l : step.libs)   cmd += " -l" + l;
    log(cmd);
    return execCmd(cmd);
}
