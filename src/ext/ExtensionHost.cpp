...
const std::vector<LoadedExtension>& ExtensionHost::loaded() const noexcept { return m_extensions; }
...
void ExtensionHost::loadDirectory(const std::filesystem::path& dir) {
    ...
    for (auto& entry : std::filesystem::directory_iterator(dir)) {
        if (entry.path().extension() == ".so") load(entry.path());
    }
}
