...
struct ConfigParser {
    ...
    void applyKeyVal(std::string_view key, std::string_view rawVal) {
        ...
        if (currentSection.empty()) {
            ...
            else if (key == "output_dir") cfg.outputDir  = val;
            else cfg.vars[std::string(key)] = val;
            return;
        }
        ...
        if (currentSection == "opt") {
            ...
            if      (key == "input")  opt.input = val;
            else if (key == "output") opt.output = val;
            else if (key == "level")  opt.level  = val;
        ...
        if (currentSection == "link") {
            if (cfg.linkSteps.empty()) return;
            auto& lnk = cfg.linkSteps.back();
            if      (key == "output") lnk.output = val;
            else if (key == "linker") lnk.linker  = val;
    ...
    LumoraConfig parse() {
        while (!atEnd()) {
            ...
            if (line_.empty() || line_.front() == '#') continue;
