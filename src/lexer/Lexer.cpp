...
namespace {
    static const std::unordered_map<std::string_view, TokenKind> kKeywords = {
        ...
        {"union",    TokenKind::KwUnion},  {"impl",     TokenKind::KwImpl},
        {"trait",    TokenKind::KwTrait},  {"type",     TokenKind::KwType},
        {"use",      TokenKind::KwUse},    {"mod",      TokenKind::KwMod},
        {"pub",      TokenKind::KwPub},    {"extern",   TokenKind::KwExtern},
        {"true",     TokenKind::KwTrue},   {"false",    TokenKind::KwFalse},
        {"null",     TokenKind::KwNull},   {"as",       TokenKind::KwAs},
        {"is",       TokenKind::KwIs},     {"match",    TokenKind::KwMatch},
        {"defer",    TokenKind::KwDefer},  {"unsafe",   TokenKind::KwUnsafe},
        {"inline",   TokenKind::KwInline}, {"volatile", TokenKind::KwVolatile},
        {"async",    TokenKind::KwAsync},  {"await",    TokenKind::KwAwait},
        {"yield",    TokenKind::KwYield},  {"sizeof",   TokenKind::KwSizeof},
        {"alignof",  TokenKind::KwAlignof},{"typeof",   TokenKind::KwTypeof},
...
void Lexer::skipWhitespaceAndComments() {
    while (true) {
        while (std::isspace(static_cast<unsigned char>(cur()))) eat();
...
std::vector<Token> Lexer::tryExtensions(size_t startPos) {
    ...
    for (auto& ext : m_extensions) {
        ...
        if (std::regex_search(remaining.data(), remaining.data() + remaining.size(), m, re)) {
            ...
            auto loc = makeLoc();
            for (size_t i = 0; i < matched.size(); ++i) eat();
...
Token Lexer::lexSymbol() {
    ...
    switch (cur()) {
        case '(': return eat1(TokenKind::LParen,    "(");
        case ')': return eat1(TokenKind::RParen,    ")");
        case '{': return eat1(TokenKind::LBrace,    "{");
        case '}': return eat1(TokenKind::RBrace,    "}");
        case '[': return eat1(TokenKind::LBracket,  "[");
        case ']': return eat1(TokenKind::RBracket,  "]");
        case ',': return eat1(TokenKind::Comma,     ",");
        case ';': return eat1(TokenKind::Semicolon, ";");
        case '~': return eat1(TokenKind::Tilde,     "~");
        case '@': return eat1(TokenKind::At,        "@");
        case '#': return eat1(TokenKind::Hash,      "#");
        case '$': return eat1(TokenKind::Dollar,    "$");
        case '?': return eat1(TokenKind::Question,  "?");

        case ':':
            if (lookahead() == ':') return eat2(TokenKind::DoubleColon, "::");
            return eat1(TokenKind::Colon, ":");
        case '.':
            if (lookahead() == '.' && lookahead(2) == '.') { eat(); eat(); eat(); return makeToken(TokenKind::Ellipsis, "...", loc); }
            if (lookahead() == '.' && lookahead(2) == '=') { eat(); eat(); eat(); return makeToken(TokenKind::DotDotEq, "..=", loc); }
            if (lookahead() == '.') return eat2(TokenKind::DotDot, "..");
            return eat1(TokenKind::Dot, ".");
        case '=':
            if (lookahead() == '=') return eat2(TokenKind::EqEq,    "==");
            if (lookahead() == '>') return eat2(TokenKind::FatArrow, "=>");
            return eat1(TokenKind::Eq, "=");
        case '!':
            if (lookahead() == '=') return eat2(TokenKind::BangEq, "!=");
            return eat1(TokenKind::Bang, "!");
        case '<':
            if (lookahead() == '<') {
                if (lookahead(2) == '=') { eat(); eat(); eat(); return makeToken(TokenKind::LtLtEq, "<<=", loc); }
                return eat2(TokenKind::LtLt, "<<");
            }
            if (lookahead() == '=') return eat2(TokenKind::LtEq, "<=");
            return eat1(TokenKind::Lt, "<");
        case '>':
            if (lookahead() == '>') {
                if (lookahead(2) == '=') { eat(); eat(); eat(); return makeToken(TokenKind::GtGtEq, ">>=", loc); }
                return eat2(TokenKind::GtGt, ">>");
            }
            if (lookahead() == '=') return eat2(TokenKind::GtEq, ">=");
            return eat1(TokenKind::Gt, ">");
        case '+':
            if (lookahead() == '+') return eat2(TokenKind::PlusPlus, "++");
            if (lookahead() == '=') return eat2(TokenKind::PlusEq,   "+=");
            return eat1(TokenKind::Plus, "+");
        case '-':
            if (lookahead() == '-') return eat2(TokenKind::MinusMinus, "--");
            if (lookahead() == '=') return eat2(TokenKind::MinusEq,    "-=");
            if (lookahead() == '>') return eat2(TokenKind::Arrow,      "->");
            return eat1(TokenKind::Minus, "-");
        case '*':
            if (lookahead() == '*') return eat2(TokenKind::StarStar, "**");
            if (lookahead() == '=') return eat2(TokenKind::StarEq,   "*=");
            return eat1(TokenKind::Star, "*");
        case '/':
            if (lookahead() == '=') return eat2(TokenKind::SlashEq, "/=");
            return eat1(TokenKind::Slash, "/");
        case '%':
            if (lookahead() == '=') return eat2(TokenKind::PercentEq, "%=");
            return eat1(TokenKind::Percent, "%");
        case '&':
            if (lookahead() == '&') return eat2(TokenKind::AmpAmp, "&&");
            if (lookahead() == '=') return eat2(TokenKind::AmpEq,  "&=");
            return eat1(TokenKind::Amp, "&");
        case '|':
            if (lookahead() == '|') return eat2(TokenKind::PipePipe, "||");
            if (lookahead() == '=') return eat2(TokenKind::PipeEq,   "|=");
            return eat1(TokenKind::Pipe, "|");
        case '^':
            if (lookahead() == '=') return eat2(TokenKind::CaretEq, "^=");
...
Token Lexer::advance() {
    ...
    if (cur() == '\0') return makeToken(TokenKind::Eof, "", makeLoc());

    auto extToks = tryExtensions(m_pos);
    if (!extToks.empty()) {
        for (size_t i = 1; i < extToks.size(); ++i) m_peekBuf.push_back(extToks[i]);
        return extToks[0];
    }

    if (std::isalpha(static_cast<unsigned char>(cur())) || cur() == '_') return lexIdOrKeyword();
    if (std::isdigit(static_cast<unsigned char>(cur()))) return lexNumber();
    if (cur() == '"')  return lexString();
    if (cur() == '\'') return lexChar();
