...
TypeRef SemaType::voidTy()  { auto t = std::make_shared<SemaType>(); t->kind = TypeKind::Void;  t->name = "void";  return t; }
TypeRef SemaType::neverTy() { auto t = std::make_shared<SemaType>(); t->kind = TypeKind::Never; t->name = "never"; return t; }
TypeRef SemaType::boolTy()  { auto t = std::make_shared<SemaType>(); t->kind = TypeKind::Bool;  t->name = "bool";  return t; }
TypeRef SemaType::charTy()  { auto t = std::make_shared<SemaType>(); t->kind = TypeKind::Char;  t->name = "char";  return t; }
TypeRef SemaType::i32Ty()   { auto t = std::make_shared<SemaType>(); t->kind = TypeKind::I32;   t->name = "i32";   return t; }
TypeRef SemaType::i64Ty()   { auto t = std::make_shared<SemaType>(); t->kind = TypeKind::I64;   t->name = "i64";   return t; }
TypeRef SemaType::u8Ty()    { auto t = std::make_shared<SemaType>(); t->kind = TypeKind::U8;    t->name = "u8";    return t; }
TypeRef SemaType::f64Ty()   { auto t = std::make_shared<SemaType>(); t->kind = TypeKind::F64;   t->name = "f64";   return t; }
...
bool SemaType::isFloat()   const noexcept { return kind == TypeKind::F32 || kind == TypeKind::F64; }
bool SemaType::isNumeric() const noexcept { return isInt() || isFloat(); }
bool SemaType::isPtr()     const noexcept { return kind == TypeKind::Pointer || kind == TypeKind::Reference; }
...
Sema::Sema() {
    ...
    for (auto& [n, k] : std::initializer_list<std::pair<const char*, TypeKind>>{
        ...
        {"i128",TypeKind::I128},{"u16",  TypeKind::U16},
        {"u32", TypeKind::U32}, {"u64",  TypeKind::U64},
        {"u128",TypeKind::U128},{"f32",  TypeKind::F32},
    }) {
        auto t = std::make_shared<SemaType>(); t->kind = k; t->name = n;
        m_typeEnv[n] = t;
    }
}
...
TypeRef Sema::primToSema(PrimTypeNode::Prim p) {
    switch (p) {
        case PrimTypeNode::Prim::I8:    return m_typeEnv["i8"];
        case PrimTypeNode::Prim::I16:   return m_typeEnv["i16"];
        case PrimTypeNode::Prim::I32:   return m_typeEnv["i32"];
        case PrimTypeNode::Prim::I64:   return m_typeEnv["i64"];
        case PrimTypeNode::Prim::I128:  return m_typeEnv["i128"];
        case PrimTypeNode::Prim::U8:    return m_typeEnv["u8"];
        case PrimTypeNode::Prim::U16:   return m_typeEnv["u16"];
        case PrimTypeNode::Prim::U32:   return m_typeEnv["u32"];
        case PrimTypeNode::Prim::U64:   return m_typeEnv["u64"];
        case PrimTypeNode::Prim::U128:  return m_typeEnv["u128"];
        case PrimTypeNode::Prim::F32:   return m_typeEnv["f32"];
        case PrimTypeNode::Prim::F64:   return m_typeEnv["f64"];
        case PrimTypeNode::Prim::Bool:  return m_typeEnv["bool"];
        case PrimTypeNode::Prim::Char:  return m_typeEnv["char"];
        case PrimTypeNode::Prim::Void:  return m_typeEnv["void"];
        case PrimTypeNode::Prim::Never: return m_typeEnv["never"];
        default:                        return m_typeEnv["void"];
    }
}
    ...
    switch (n.kind) {
        ...
        case NodeKind::NamedType: {
            ...
            if (!named.path.empty()) {
                ...
                if (it != m_typeEnv.end()) return it->second;
                auto t = std::make_shared<SemaType>();
...
void Sema::analyzeStmt(Node& n) {
    switch (n.kind) {
        ...
        case NodeKind::IfStmt: {
            ...
            if (s.thenBranch) analyzeStmt(*s.thenBranch);
...
TypeRef Sema::analyzeExpr(Node& n) {
    switch (n.kind) {
        case NodeKind::IntLit:    return SemaType::i64Ty();
        case NodeKind::FloatLit:  return SemaType::f64Ty();
        case NodeKind::BoolLit:   return SemaType::boolTy();
        case NodeKind::CharLit:   return SemaType::charTy();
        case NodeKind::NullLit:   return SemaType::ptrTy(SemaType::voidTy());
        ...
        case NodeKind::UnaryExpr: {
            ...
            if (u.op == TokenKind::Bang) return SemaType::boolTy();
            if (u.op == TokenKind::Star) return ty && ty->inner ? ty->inner : SemaType::voidTy();
            if (u.op == TokenKind::Amp)  return SemaType::refTy(ty);
            return ty;
        }
        ...
        case NodeKind::ExtensionNode: {
            ...
            for (auto& ep : m_extensions) {
                if (ep.extensionId == ext.extensionId) return ep.handler(ext, *this);
            }
